...
def mock_project_service():
    ...
    service.get_folder_sort_order = Mock(side_effect=lambda x: {
        "pre-edit": 0,
        "post-edit": 1,
        "post-edit2": 2,
        "correct-edit": 3
    ...
    service.get_archive_name = Mock(side_effect=lambda parent, name: 
        f"{name.replace('-', '')}_{parent.replace('-', '')}.zip"
    )
    
    service.get_docker_tag = Mock(side_effect=lambda parent, name:
        f"{name.replace('-', '')}:{parent.replace('-', '')}"
    )
...
def mock_platform_service():
    ...
    service.create_archive_command = Mock(return_value=(["zip", "-r", "archive.zip", "."], False))
    service.create_bash_command = Mock(return_value=(["bash", "-c", "echo test"], 'bash -c "echo test"'))
