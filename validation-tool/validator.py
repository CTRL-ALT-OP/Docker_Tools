...
class SimpleValidator:
        ...
        for i in range(1, len(parts)):
            ...
            if current_path.exists() and current_path.is_file():
                ...
                while backup_path.exists():
                    backup_path = current_path.with_suffix(f"{current_path.suffix}.bak{counter}")
                    counter += 1
    ...
    def extract_codebase(self, zip_path: Path) -> Tuple[Path, float]:
        ...
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            ...
            if windows_paths:
                print(f"Found {len(windows_paths)} Windows-style paths, using manual extraction...")
                ...
                try:
                    ...
                    if "Not a directory" in str(e):
                        ...
                        print("Falling back to manual extraction with conflict resolution...")
                        self._extract_with_conflict_resolution(zip_ref, extract_dir)
                    else:
                        raise
    ...
    def _extract_with_conflict_resolution(self, zip_ref: zipfile.ZipFile, extract_dir: Path):
        ...
        for member in zip_ref.infolist():
            ...
            try:
                ...
                print(f"Directory creation failed: {e}, attempting conflict resolution...")
                self._resolve_path_conflicts(target_path.parent)
                target_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def build_docker_image(self, extract_dir: Path, image_name: str) -> Tuple[bool, str, str, float]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if image_exists:
                    ...
                    print(f"✗ Build script succeeded but image not found: {image_name} ({build_time:.1f}s)")
                    return False, result.stdout, result.stderr + "\nError: Docker image was not created despite successful build script", build_time
            else:
    ...
    def validate_codebase(self, zip_path: Path) -> Dict:
        ...
        try:
            ...
            # Build Docker image
            build_success, build_stdout, build_stderr, build_time = self.build_docker_image(extract_dir, image_name)
            ...
            # Run tests
            test_success, test_stdout, test_stderr, exec_time = self.run_tests(image_name, extract_dir)
    ...
    def validate_codebase_with_progress(self, zip_path: Path, progress, codebase_type: str = 'rewrite') -> Dict:
        ...
        try:
            ...
            if codebase_type == 'rewrite':
                ...
                result["validation_success"] = result["build_success"] and result["test_success"]
            else:
            ...
            if result["validation_success"]:
                if codebase_type == 'rewrite':
                    ...
                    progress.message = f"Validation successful - Build passed ({codebase_type})"
    ...
    def save_results_csv(self, output_path: str = "validation_results.csv"):
        ...
        fieldnames = [
            "codebase_name", "zip_file", "timestamp", "extraction_success",
            "build_script_present", "test_script_present", "build_success", 
            "test_success", "test_execution_time", "docker_image", "error_message",
            "build_output", "build_error", "test_output", "test_error",
            "build_time", "extraction_time"
        ]
...
def main():
    ...
    try:
        ...
        print("Error: Docker is not available. Please install Docker and ensure it's running.")
        sys.exit(1)
